...
window.document = {
    ...
    cookie:'wxa_level=1; cid=9; jxsid=17345980668639863687; webp=1; __jdv=122270672%7Cdirect%7C-%7Cnone%7C-%7C1734598066876; mba_muid=1734598066875666927197; visitkey=8568091772987487103; __jdc=122270672; 3AB9D23F7A4B3C9B=UB5II2YGB2IQ55UEBLIET2RRUU4774FIRQIZEPBZCAQEIL5EN3OZ7OBSHM6TZL4ZIHPJRBQYVDUNNIIDP275MECYVA; cd_eid=jdd03UB5II2YGB2IQ55UEBLIET2RRUU4774FIRQIZEPBZCAQEIL5EN3OZ7OBSHM6TZL4ZIHPJRBQYVDUNNIIDP275MECYVAAAAAMT3ZAHCOIAAAAADOR6EMPJ5IT3MYX; __jda=122270672.1734598066875666927197.1734598066.1734598066.1734600585.2; share_cpin=; share_open_id=; share_gpin=; shareChannel=; source_module=; erp=; sc_width=2048; shshshfpa=1cf0686b-1783-f773-c76d-50378150cd2b-1734600585; shshshfpx=1cf0686b-1783-f773-c76d-50378150cd2b-1734600585; appCode=ms0ca95114; jxsid_s_u=https%3A//so.m.jd.com/list/couponSearch.action; jcap_dvzw_fp=xfUBzGVNLJR8kwn2J3KlYoTVoEqiyf00WPX14uYYseAPbeLlBTyJYOCFgvMv4e_M_cNN5fBk8EZSAPMOSsTTnw==; pt_token=hdgc71rv; pwdt_id=jd_uTaLohrdhwQz; sfstoken=tk01mc5671c93a8sMysyeDMrMisxFL4klk4m9Ew8EMMg88LC3X0m+sLspPINfKylLMWf29ygdgWLpbCf9WulXNR5UVIu; warehistory="100142621568,"; 3AB9D23F7A4B3CSS=jdd03UB5II2YGB2IQ55UEBLIET2RRUU4774FIRQIZEPBZCAQEIL5EN3OZ7OBSHM6TZL4ZIHPJRBQYVDUNNIIDP275MECYVAAAAAMT3ZKH5IQAAAAACLH3QBEGT4QU4IX; PPRD_P=UUID.1734598066875666927197-LOGID.1734601899675.1561125325; autoOpenApp_downCloseDate_autoOpenApp_autoPromptly=1734601899709_1; __wga=1734601900573.1734600585130.1734600585130.1734600585130.7.1; shshshfpb=BApXSvxNc3fZABsZ75cQZx3WkOUA6lnzxBnYhAqpo9xJ1PdZfQpPikzbkhDn-O5FCaV_ftabn; jxsid_s_t=1734601901132; retina=1; wqmnx1=MDEyNjM1MXQvbWNpb1NoaW9iPTMwNjc3MGk1TDtyNk4gaVIpbEszIE1pZSBtMS5iU2kuZDEuMnM0VS01MU9DSCooKQ%3D%3D; __jdb=122270672.19.1734598066875666927197|2.1734600585; mba_sid=1734598066877557821615064872.24',
    head:wacth({
...
function readLocalStorage() {
    if (fs.existsSync(localStorageFile)) {
        const fileData = fs.readFileSync(localStorageFile, 'utf8');
        try {
            return fileData ? JSON.parse(fileData) : {};  // 解析 JSON 或返回空对象
        } catch (error) {
            console.error("读取文件时发生错误，返回空对象：", error);
            return {};  // 解析失败时返回空对象
        }
    }
    return {};  // 文件不存在时返回空对象
...
// 获取或生成 WQ_gather_cv1
function getWQGatherCv1() {
    let data = readLocalStorage();  // 读取缓存文件
    let wqGatherCv1 = data["WQ_gather_cv1"];

    if (!wqGatherCv1) {
        // 如果缓存中没有 WQ_gather_cv1，生成新的
        wqGatherCv1 = JSON.stringify({
            v: calculateMD5(`${getRandomInt10()}`),
            t: Date.now(),
            e: 31536000  // 设定有效期（例如一年）
        });
        data["WQ_gather_cv1"] = wqGatherCv1;  // 更新缓存
        writeLocalStorage(data);  // 写入缓存文件
    }

    return wqGatherCv1;
}
const wqGatherCv1=getWQGatherCv1()
console.log("wq_Gather_Cv1",wqGatherCv1)
...
var ParamsSign = function() {
    ...
    _$o8.prototype.sign = function(_$on) {
        ...
        try {
            ...
            console.log("_$oF.length",_$oF.length)
...
async function getH5st(args) {
    ...
    //   }
    try {
        const e = await window.Psign.sign(r);
